...
def now_utc() -> datetime:  # Helper: return timezone-aware UTC datetime for timestamps
...
class GenerativeTruthValue(Enum):  # Multi-valued generative truth (G_HINGE..G_TRANSCENDENT)
    """
...
class LogicalOperator(ABC):  # Abstract base class for logical operators
...
class GenerativeZeroOperator(LogicalOperator):  # Operator metabolizing contradictions into possibility sets (⊙₀)
    """
...
class GenerativeNegationOperator(LogicalOperator):  # Operator implementing generative negation (∇_g)
    """
...
class MetabolicCompositionOperator(LogicalOperator):  # Operator for metabolic composition (⊗)
    """
